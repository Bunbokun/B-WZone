...
//        System.out.println("A:" +A.size() + " B: " + B );
    }
    ...
    public ArrayList<ArrayList<Path>> coverage() {
        ...
        while (i >= 0) {
            ...
            System.out.printf("closest cell " + m.c + ", distance " + m.c.getDistance() + ", B = " + B +"%n");
            m.c.printCell();
            B1 -= m.c.getDistance();
//            System.out.printf("B1: " + B1);
            ...
            Path r = remains(t);
            if (r != null) {
                System.out.println("Remain Coverage working zone " + (A.size() - i));
                P.add(r);
                P2.add(r);
...
//            Row firstrow = e.getRows().get(0);
//            int size = firstrow.getCells().size() -1;
//            m = new Move(firstrow.getCells().get(size), t);
    ...
    private Path remains(Tree t) {
        ...
        Path p = cover(m.c, t, m.t, B - m.c.getDistance());
        if (getClosetCell(t) == null) {
            return p;
        }
        for (Cell c : p.cells)
